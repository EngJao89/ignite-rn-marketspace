...
export function MyAd() {
  ...
  const [myProductByFilter, setMyProductByFilter] = useState<
    ProductDetailsDTO[]
  >([])
  ...
  async function fetchMyAd() {
    try {
      ...
      const title = isAppError
        ? error.message
        : 'Não foi carregar seus anúncios. Tente novamente mais tarde.'
  ...
  function handleLoadProductsByFilter() {
    if (filter === 'ativos') {
      const productsActive = myProductsList.filter(
        (product) => product.is_active === true,
      )
      setMyProductByFilter(productsActive)
    } else if (filter === 'inativos') {
      const productsInactive = myProductsList.filter(
        ...
        <VStack bg="gray.600" flex={1}>
          ...
          >
            ...
            <Select
              ...
              px={3}
              dropdownCloseIcon={
                <Icon as={MaterialIcons} name="expand-more" mr={2} size={5} />
              }
              dropdownOpenIcon={
                <Icon as={MaterialIcons} name="expand-less" mr={2} size={5} />
              }
              placeholder="Todos"
